...
class CodeExtractor:
		...
		def __init__(self, directory):
				...
				self.base = 'https://api.openai.com/v1/completions'
				self.COMPLETIONS_MODEL = "chat-davinci-003-alpha"
				self.headers = {
						'Content-Type': 'application/json',
						'Authorization': 'Bearer ' + api_key,
				}

				self.MAX_SECTION_LEN = 950
				self.SEPARATOR = "\n* "
				self.ENCODING = "gpt2"
		...
		def split_code_by_lines(self, df: pd.DataFrame, max_lines: int = 3) -> pd.DataFrame:
				new_rows = []
...
""" 		def gpt_4(self, prompt=""):
				codebaseContext = self.construct_prompt(question=prompt)
				print(codebaseContext)

				prompt = (
						f"SYSTEM: You are the ASSISTANT helping the USER with a typescript react project startup.\n"
						f"You are intelligent, helpful, and an expert developer, who always gives the correct answer and only does what is instructed. "
...
USER: Clean up the code and make it more readable for the users.

extractor = CodeExtractor(f"{root_dir}/Downloads/whopt")
df = extractor.get_files_df()
df = extractor.split_code_by_lines(df, 20)
extractor.df = df
...
def ask(query):
	print("\x1b[40;38;5;82m" + "You, the User\t" + "AI, the Assistant" + "\x1b[0m")
	question = input("\nUSER:\t")
	if question == "q":
		print("bye")
		exit(0)
		...
		try:
			extractor.chatbot(prompt=f"USER: {question}")
		except:
